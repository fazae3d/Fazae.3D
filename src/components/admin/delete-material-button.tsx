...
import { deleteMaterialAction } from "@/app/admin/materiais/actions";

export function DeleteMaterialButton({ slug }: { slug: string }) {
    ...
    return (
      <span className="flex items-center gap-2 text-[11px]">
        ...
        <button
          ...
          onClick={async () => {
            setPending(true);
            const result = await deleteMaterialAction(slug);
            if (!result.success) {
